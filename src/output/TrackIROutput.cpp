...
bool TrackIROutput::initialize() {
    ...
    if (m_hMapFile == NULL) {
        std::cerr << "Failed to create TrackIR shared memory. Error: " << GetLastError() << std::endl;
        return false;
    }
    ...
    if (m_pMemory == nullptr) {
        std::cerr << "Failed to map TrackIR shared memory. Error: " << GetLastError() << std::endl;
...
bool TrackIROutput::sendData(const TrackingData& data) {
    if (!m_isInitialized) {
        return false;
    }
    ...
    TrackIRData* tirData = static_cast<TrackIRData*>(m_pMemory);
